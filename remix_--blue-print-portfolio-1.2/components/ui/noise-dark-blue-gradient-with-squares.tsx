...
const Noise: React.FC<{ refresh?: number; alpha?: number }> = ({ refresh = 2, alpha = 18 }) => {
  ...
  useEffect(() => {
    ...
    const frames = [draw(), draw(), draw(), draw()];

    const loop = () => {
      if (f % refresh === 0) {
        ctx.putImageData(frames[(f / refresh) % frames.length], 0, 0);
      }
      f++;
  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      if (vignette) {
        ...
        grad.addColorStop(1, "rgba(6,0,16,0.6)");
        ctx.fillStyle = grad;
...
export default function Component({
  ...
  hoverStrokeColor = "rgba(255,255,255,0.1)",
  hoverGlowColor   = "rgba(255,255,255,0.05)", 
  interactive = true,
  className = "",
}: CombinedProps) {
  ...
  return (
    <div className={`fixed inset-0 z-0 bg-[#002FA7] ${className}`}>
      {/* soft spotlight tone */}
      <div className="absolute inset-0 z-0 bg-[radial-gradient(circle_1000px_at_50%_50%,rgba(37,99,235,0.35),transparent_80%)]" />
      ...
      {/* interactive hover cell (optional) */}
      {interactive && (
      ...
      <div className="pointer-events-none absolute inset-0 z-40 bg-gradient-to-b from-transparent via-transparent to-black/30" />
    </div>
  );
}
